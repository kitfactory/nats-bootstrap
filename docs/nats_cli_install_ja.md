...
Language: [English](nats_cli_install.md)

この文書は **nats-bootstrap の backup/restore 用**の `nats` CLI 導入手順をまとめたものです。  
`up` / `join` / `status` / `doctor` には不要です。
...
戻る: [日本語 README](../README_ja.md) | [日本語詳細マニュアル](../manuals/manual_ja.md) | [English](nats_cli_install.md)
